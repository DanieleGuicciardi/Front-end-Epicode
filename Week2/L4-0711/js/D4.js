...
let frase = "e un istituzione!"

function epify(p="EPICODE ") {
    if (frase.indexOf(p) === 0) {
        return frase;
    } else {
        frase = p + frase;
        return frase;
...
function check3and7(x) {
    if (x % 3 == 0 || x % 7 == 0) {
        ...
        return "il numero non e multiplo di 3 o 7";
    }
}
...
function upperFirst(p) {
    let parole = p.split(" ");
    let maiuscParole = parole.map(parole => {
        return parole.charAt(0).toUpperCase() + parole.slice(1);
    });
    return maiuscParole.join(" ");
}
